...
namespace Core
{
    public class ExtNetCore
    {
        ...
        /***********创建Web层的UI页面****************/
        public static ArrayList CreateUIElement(DataTable dt, List<ComBoxStore> list, string AutoID, int cols)
        {
            try
            {
                    ...
                    if (dr[0].ToString() == "true" || dr["字段名"].ToString() != AutoID)
                    {
                        ...
                        switch (dr["Form"].ToString())
                        {
                            ...
                            case "TextArea":
                                ...
                                Designer.AppendLine("\t\tprotected global::Ext.Net.TextArea TA" + dr["字段名"].ToString() + ";");
                                ModelSet.AppendLine("\t\t\t_{TABLENAME}." + dr["字段名"].ToString() + " = TA" + dr["字段名"].ToString() + ".Text; //" + dr["说明"].ToString());
                                break;
                            case "ComboBox":
                                ComBoxStore cbo = null;
                                if (list.Count > 0)
                                    cbo = list.First<ComBoxStore>(delegate(ComBoxStore l) { return l.FieldName == dr["字段名"].ToString(); });
                            ...
                            case "MultiCombo":
                                ...
                                if (list.Count > 0)
                                    mcbo = list.First<ComBoxStore>(delegate(ComBoxStore l) { return l.FieldName == dr["字段名"].ToString(); });
                                ...
                                ModelSet.AppendLine("\t\t\t//_{TABLENAME}." + dr["字段名"].ToString() + " = MCbo" + dr["字段名"].ToString() + ".Text; //" + dr["说明"].ToString());
                                break;
                        }
                        Cells.AppendLine("\t\t\t\t\t\t\t\t\t</Items>");
        ...
        public static string CreateAspx(ArrayList list, string path, string UIPath, string ModelName, string AutoID, int cols)
        {
            try
            {
